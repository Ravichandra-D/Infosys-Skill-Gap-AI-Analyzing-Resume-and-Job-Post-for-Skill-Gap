...
# Configure page
st.set_page_config(
    page_title="AI Skill Gap Analyzer Project",
    page_icon="🎯",
    layout="wide",
...
st.markdown("""
    <style>
    :root{
        --bg:#120423; /* deep purple background */
        --card:#27102f; /* card surface */
        --muted:#c6a0d6;
        --accent:#ff6b6b; /* coral accent */
        --success:#10b981; --warning:#f59e0b; --danger:#ef4444; --glass: rgba(255,255,255,0.03);
    }

    /* Page background and container adjustments */
    .stApp > div:first-child { background: linear-gradient(180deg, var(--bg), #041f2a); color: #e6f7f6; }
    .css-1d391kg { background: linear-gradient(180deg, rgba(4,30,38,0.65), rgba(3,18,24,0.7)); }
    ...
    /* Match badges */
    .strong-match { background-color: rgba(16,185,129,0.12); color: var(--success); padding: 8px 12px; border-radius: 8px; margin: 5px; display:inline-block }
    .partial-match { background-color: rgba(6,182,212,0.08); color: var(--accent); padding: 8px 12px; border-radius: 8px; margin: 5px; display:inline-block }
    .missing-skill { background-color: rgba(239,68,68,0.08); color: var(--danger); padding: 8px 12px; border-radius: 8px; margin: 5px; display:inline-block }
    ...
    div.stButton > button:hover, div.stDownloadButton > button:hover { transform: translateY(-2px); box-shadow: 0 14px 34px rgba(6,182,212,0.18); }

    /* Sidebar tweaks */
    .css-1d391kg header, .css-1d391kg .stSidebar { background: linear-gradient(180deg,#041622, #03201e); }
...
class GapVisualizer:
    """Create visualizations for gap analysis (enhanced styles + additional charts)"""
    
    @staticmethod
    def create_similarity_heatmap(similarity_matrix: np.ndarray,
                                 resume_skills: List[str],
                                 jd_skills: List[str]) -> go.Figure:
        """Create interactive similarity heatmap with improved colors and hover info"""
        max_display = 25
        ...
        # Custom diverging colorscale (red -> white -> blue)
        colorscale = [
            [0.0, '#b2182b'],
            [0.25, '#ef8a62'],
            [0.5, '#f7f7f7'],
        ...
        heat = go.Heatmap(
            ...
            zmin=0.0,
            zmax=1.0,
            colorbar=dict(title=dict(text="Similarity", side="right"), tickmode="linear", tick0=0, dtick=0.2),
        ...
        fig.update_layout(
            ...
            title=dict(text=f"Skill Similarity Heatmap (Top {min(max_display, len(resume_skills))} x {min(max_display, len(jd_skills))} skills)", x=0.5),
    ...
    def create_match_distribution_pie(analysis_result: GapAnalysisResult) -> go.Figure:
        """Create a stylized donut chart for match distribution"""
        ...
        fig.update_layout(template='plotly_dark', title='Skill Match Distribution', height=420)
        return fig

    @staticmethod
    def create_skill_comparison_bar(analysis_result: GapAnalysisResult, top_n: int = 15) -> go.Figure:
        """Horizontal bar chart with gradient-like coloring and labels"""
        all_matches = (analysis_result.matched_skills + analysis_result.partial_matches + analysis_result.missing_skills)
        ...
        # Color mapping using thresholds to keep it simple and compatible
        def col_for_sim(s):
            if s >= 80: return '#16a34a'
            if s >= 50: return '#f59e0b'
            return '#ef4444'
        colors = [col_for_sim(s) for s in similarities]

        fig = go.Figure(go.Bar(y=skills, x=similarities, orientation='h', marker=dict(color=colors, line=dict(color='#0b1220', width=1.5)),
                               text=[f"{s:.1f}%" for s in similarities], textposition='outside'))
        fig.update_layout(template='plotly_dark', title=f"Top {top_n} Skills by Similarity Score", xaxis_title="Similarity (%)", height=560, yaxis=dict(autorange='reversed'))
        return fig
    ...
    def create_gap_priority_chart(missing_skills: List[SkillMatch]) -> go.Figure:
        ...
        priority_colors = {'HIGH': '#ef4444', 'MEDIUM': '#f59e0b', 'LOW': '#16a34a'}
        colors = [priority_colors.get(p, '#94a3b8') for p in priorities]

        fig = go.Figure(data=[go.Bar(y=skills, x=similarities, orientation='h', marker=dict(color=colors), text=[f"{s:.1f}% - {p}" for s, p in zip(similarities, priorities)], textposition='auto')])
        fig.update_layout(template='plotly_dark', title='Missing Skills by Priority', xaxis_title='Current Similarity (%)', height=480, yaxis=dict(autorange='reversed'))
        return fig
    ...
    def create_overall_score_gauge(overall_score: float) -> go.Figure:
        ...
        fig = go.Figure(go.Indicator(
            ...
            gauge={
                ...
                'bar': {'color': '#7c3aed'},
                'steps': [
                    {'range': [0, 40], 'color': '#38111a'},
                    {'range': [40, 70], 'color': '#4b2f23'},
                    {'range': [70, 100], 'color': '#123b22'}
                ],
                'threshold': {'line': {'color': "#ff004c", 'width': 4}, 'value': 70}
            }
        ))
        
        fig.update_layout(template='plotly_dark', height=360, margin=dict(l=20, r=20, t=40, b=20))
        
        return fig

    @staticmethod
    def create_similarity_scatter(analysis_result: GapAnalysisResult) -> go.Figure:
        """Scatter showing best-match similarity per JD skill"""
        sim_mat = analysis_result.similarity_matrix
        ...
        for j, jd in enumerate(jd_skills):
            best_i = int(sim_mat[:, j].argmax())
            best_sim = float(sim_mat[best_i, j])
            pts.append(best_sim * 100)
            labels.append(f"JD: {jd}<br>Resume: {resume_skills[best_i]}<br>Sim: {best_sim:.2f}")
            if best_sim >= 0.8:
        ...
        color_map = {'Strong': "#8b16a3", 'Partial': '#f59e0b', 'Missing': '#ef4444'}
        fig = go.Figure()
        for cat in ['Strong', 'Partial', 'Missing']:
            inds = [i for i, c in enumerate(categories) if c == cat]
            fig.add_trace(go.Scatter(x=[pts[i] for i in inds], y=[jd_skills[i] for i in inds], mode='markers', marker=dict(size=[8 + (pts[i]/12) for i in inds], color=color_map[cat], line=dict(color='#0b1220', width=1.2)), name=cat, text=[labels[i] for i in inds], hoverinfo='text'))

        fig.update_layout(title='Best-match Similarity per JD Skill', xaxis_title='Similarity (%)', template='plotly_dark', height=600, yaxis=dict(autorange='reversed'))
        return fig

    @staticmethod
    def create_category_treemap(analysis_result: GapAnalysisResult) -> go.Figure:
        """Treemap of category scores to show where strengths/weaknesses lie"""
        cat_scores = analysis_result.category_scores or {}
        if not cat_scores:
            # Fallback: use counts
            cat_counts = defaultdict(int)
            for m in analysis_result.matched_skills: cat_counts['Matched'] += 1
            for p in analysis_result.partial_matches: cat_counts['Partial'] += 1
            for q in analysis_result.missing_skills: cat_counts['Missing'] += 1
        ...
        fig = go.Figure(go.Treemap(labels=labels, values=values, marker=dict(colors=values, colorscale='Viridis')))
        fig.update_layout(title='Category Scores / Distribution', template='plotly_dark', height=500)
        return fig
...
class CompleteSkillGapApp:
    """Complete Project Streamlit Application"""
    ...
    def run(self):
        ...
        st.title("🎯Intelligent AI Skill Extraction Dashboard 🎯 ")
        st.markdown("### Contextual skill discovery and prioritised growth recommendations using semantic embeddings")
    ...
    def _gap_analysis_tab(self):
        ...
        st.header("Skills Discovery & Matching")
        
        st.markdown("""
        **Overview:**
        1. Supply resume abilities and role expectations (manual, upload or prefilled samples).
        2. The engine generates contextual embeddings to understand meaning beyond exact phrase matches.
        3. It computes semantic similarity across all resume↔JD skill pairs.
        4. Classifies skills as strong alignments, partial overlaps, or gaps and ranks them by priority.
        5. Get personalized learning steps, visual summaries and exportable reports.
        """)
        
        # Input method selection
        input_method = st.radio(
            "Choose input method:",
            ["Manual Entry", "Upload ", "Sample Data"],
            horizontal=True
        )
    ...
    def _visualizations_tab(self):
        """Insights tab - interactive charts and overview"""
        ...
        with c2:
            ...
            fig_treemap = self.visualizer.create_category_treemap(result)
        ...
        # Full width scatter showing best matches per JD
        st.subheader("Best-match Similarity per JD Skill")
        fig_scatter = self.visualizer.create_similarity_scatter(result)
    ...
    def _settings_tab(self):
        ...
        st.header("🔧 Preferences & Configuration")
...
def main():
    ...
    try:
        ...
        with st.sidebar:
            ...
            st.markdown("---")
            st.caption(" Complete Implementation")
